...
MillisecondTimer::MillisecondTimer(const unsigned int millis) : expiry(
    timespec_now()) {
...
Serial::Serial()
  : fd_(-1), is_open_(false), xonxoff_(false), rtscts_(false),
    parity_(parity_none),
...
bool Serial::open(const char *port, unsigned long baudrate) {
  ...
  baudrate_ = baudrate;
  fd_ = ::open(port, O_RDWR | O_NOCTTY | O_NONBLOCK | O_APPEND | O_NDELAY);
...
bool Serial::setStandardBaudRate(speed_t baudrate) {
#ifdef __linux__
...
bool Serial::setCustomBaudRate(unsigned long baudrate) {
  struct termios2 tio2;
...
bool Serial::setTermios(const termios *tio) {
...
bool Serial::getTermios(termios *tio) {
